...
class GeminiService:
    def __init__(self, api_key: Optional[str] = None):
        ...
        # Configure generation parameters with low-hallucination settings
    ...
    async def generate_response(self, message: str, model: str = "gemini-2.5-flash") -> str:
        """
        Asynchronously generates content using the Gemini client and KissanAI configuration.
        """
        if not self.client:
            raise ValueError("Gemini API key is not configured. Set GEMINI_API_KEY in your environment.")

        # Async generation call using client.aio
