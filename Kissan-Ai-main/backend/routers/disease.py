...
async def detect_disease(
    ...
    # --- Save image record ---
    image = Image(user_id=current_user.id, image_url=cloudinary_result.build_url(secure=True), image_type="disease")
    db.add(image)
    await db.commit()
    ...
    try:
        disease_name, confidence = await asyncio.to_thread(predict_disease, contents)
    except Exception:
