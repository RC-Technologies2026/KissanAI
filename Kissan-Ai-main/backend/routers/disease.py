...
logger = logging.getLogger("kissanai.disease")
...
async def detect_disease(
    ...
    try:
        ...
        # diagnose_leaf_image should never raise (it handles internally),
        # but just in case — return graceful fallback.
        logger.error("Disease detection unexpected raise: %s", gemini_err, exc_info=True)
    ...
    if isinstance(result, DiseaseFallbackResponse):
        logger.warning("Disease detection: FallbackResponse — %s", result.message)
    ...
    if isinstance(result, dict):
        logger.warning("Disease detection: plain dict fallback — %s", result.get("message", "?"))
        return DiseaseDetectionResponse(
            id=None,
            image_id=image.id,
            crop_name=crop_name or "Unknown crop",
    ...
    logger.info("Disease detection success: model=%s", model_used)
